...
| --- | --- | --- |
| Domain, persistence, components, WebMCP adapter, and Inworld helper | `npm test` | **45 tests passed** |
...
- All four tools execute against a freshly reset room.
...
2. Discover the four tools; record their exact names and schemas.
...
6. Click **Approve test**; expect state version `14` and an approved test that has not run.
7. Call `propose_release_decision` with `expectedStateVersion: 14`, `recommendation: hold`, and `testProposalId: P-017`; expect pending `P-018` while the header remains `UNDECIDED`.
8. Replay the exact same request; expect `replayed: true` and unchanged state version `15`.
9. Send a stale proposal with `expectedStateVersion: 12`; expect `state_conflict` and unchanged proposal counts.
10. Click **Confirm HOLD**; expect `HOLD`, `Human confirmed`, and a final state version `16`.
...
- Execution of the proposed regression test against a payment service.
